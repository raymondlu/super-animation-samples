...
MoreSample* MoreSample::create(){
	MoreSample *pRet = new MoreSample();
	if (pRet && pRet->init())
    {
        pRet->autorelease();
        return pRet;
...
bool MoreSample::init(){
	...
	CCSprite* aSpriteBackground = CCSprite::create("HelloWorld.png");
	this->addChild(aSpriteBackground);
	aSpriteBackground->setPosition(ccp(aScreenSize.width * 0.5f, aScreenSize.height * 0.5f));
